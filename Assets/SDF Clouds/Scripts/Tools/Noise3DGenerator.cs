...
public class Noise3DGenerator : MonoBehaviour
{
    ...
    [SerializeField] private Vector3Int worleyResolution;
    ...
    [Header("Vertical attenuation (3D only)")]
    [SerializeField] private bool attenuateVertically;
    [SerializeField] private float attenuateTopExponent;
    [SerializeField] private float attenuateBottomExponent;
    ...
    /// </summary>
    /// <param name="outputPath">Must be the path & name of the file WITHOUT extension</param>
    public void Generate2DWorley_GPU(Vector2Int resolution, List<Vector3> points, float attenuation, float radius, int octaves, string outputPath)
    {
    ...
    public RenderTexture ComputeWorleyTexture3D(List<Vector3> worleyPoints, Vector3Int textureSize, float attenuation, float radius, int octaves)
    {
        ...
        worleyCompute3D.SetInts("textureDimensions", textureSize.x, textureSize.y, textureSize.z);
        ...
        worleyCompute3D.SetBool("attenuate_vertically", attenuateVertically);
        worleyCompute3D.SetFloat("attenuate_top_exponent", attenuateTopExponent);
        worleyCompute3D.SetFloat("attenuate_bottom_exponent", attenuateBottomExponent);


        worleyCompute3D.Dispatch(computeKernel, textureSize.x / 8, textureSize.y / 8, textureSize.z / 8);

        pointsBuffer.Release();
        return resRenderTex;
    }

    public RenderTexture ComputeWorleyTexture2D(List<Vector3> worleyPoints, Vector2Int textureSize, float attenuation, float radius, int octaves)
    {
        ...
        worleyCompute2D.SetInts("textureDimensions", textureSize.x, textureSize.y, 1);
        ...
        worleyCompute2D.Dispatch(computeKernel, textureSize.x / 8, textureSize.y / 8, 1);
    ...
    private void Update()
    {
        ...
        if (computeWorley)
        {
            ...
            {
                Generate2DWorley_GPU(new Vector2Int(worleyResolution.x, worleyResolution.y), points, worleyAttenuation, worleyRadius, worleyOctaves, worleyOutputPath);
            }
